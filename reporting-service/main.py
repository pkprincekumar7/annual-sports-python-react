...
app = FastAPI(title="Reporting Service", version="0.1.0", docs_url=None, redoc_url=None)
...
async def registration_deadline_middleware(request: Request, call_next):
    if request.url.path.startswith("/reportings"):
...
app.include_router(export_router.router, prefix="/reportings")


@app.get("/reportings/swagger.yaml", include_in_schema=False)
async def swagger_spec():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/reportings/swagger.yml", include_in_schema=False)
async def swagger_spec_alias():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/reportings/docs", include_in_schema=False)
async def swagger_ui():
    return HTMLResponse(
        get_swagger_ui_html(
            openapi_url="/reportings/swagger.yaml",
            title="Reporting Service API Docs",
        ).body
    )
