...
app = FastAPI(title="Enrollment Service", version="0.1.0", docs_url=None, redoc_url=None)
...
async def registration_deadline_middleware(request: Request, call_next):
    if request.url.path.startswith("/enrollments"):
...
app.include_router(batches_router.router, prefix="/enrollments")


@app.get("/enrollments/swagger.yaml", include_in_schema=False)
async def swagger_spec():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/enrollments/swagger.yml", include_in_schema=False)
async def swagger_spec_alias():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/enrollments/docs", include_in_schema=False)
async def swagger_ui():
    return HTMLResponse(
        get_swagger_ui_html(
            openapi_url="/enrollments/swagger.yaml",
            title="Enrollment Service API Docs",
        ).body
    )
