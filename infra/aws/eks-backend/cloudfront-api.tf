...
locals {
  apigw_origin_domain = var.cloudfront_enabled ? replace(aws_apigatewayv2_api.http[0].api_endpoint, "https://", "") : ""
}
...
resource "aws_cloudfront_distribution" "api" {
  ...
  dynamic "logging_config" {
    for_each = var.cloudfront_enabled && var.cloudfront_logging_enabled && var.cloudfront_logs_bucket_name != "" ? [1] : []
    content {
