...
app = FastAPI(title="Sports Participation Service", version="0.1.0", docs_url=None, redoc_url=None)
...
async def registration_deadline_middleware(request: Request, call_next):
    if request.url.path.startswith("/sports-participations"):
...
app.include_router(sports_router.router, prefix="/sports-participations")
app.include_router(captains_router.router, prefix="/sports-participations")
app.include_router(coordinators_router.router, prefix="/sports-participations")
app.include_router(teams_router.router, prefix="/sports-participations")
app.include_router(participants_router.router, prefix="/sports-participations")


@app.get("/sports-participations/swagger.yaml", include_in_schema=False)
async def swagger_spec():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/sports-participations/swagger.yml", include_in_schema=False)
async def swagger_spec_alias():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/sports-participations/docs", include_in_schema=False)
async def swagger_ui():
    return HTMLResponse(
        get_swagger_ui_html(
            openapi_url="/sports-participations/swagger.yaml",
            title="Sports Participation Service API Docs",
        ).body
    )
