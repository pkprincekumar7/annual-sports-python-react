...
app = FastAPI(title="Event Configuration Service", version="0.1.0", docs_url=None, redoc_url=None)
...
async def registration_deadline_middleware(request: Request, call_next):
    if request.url.path.startswith("/event-configurations"):
...
app.include_router(event_years_router.router, prefix="/event-configurations")


@app.get("/event-configurations/swagger.yaml", include_in_schema=False)
async def swagger_spec():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/event-configurations/swagger.yml", include_in_schema=False)
async def swagger_spec_alias():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/event-configurations/docs", include_in_schema=False)
async def swagger_ui():
    return HTMLResponse(
        get_swagger_ui_html(
            openapi_url="/event-configurations/swagger.yaml",
            title="Event Configuration Service API Docs",
        ).body
    )
