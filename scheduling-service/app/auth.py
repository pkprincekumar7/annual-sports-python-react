...
from .external_services import get_identity_me


logger = logging.getLogger("scheduling-service.auth")
...
async def authenticate_token(request: Request) -> Optional[Dict[str, Any]]:
    ...
    try:
        user = await get_identity_me(token)
    except Exception as exc:
        logger.debug("Identity lookup failed: %s", exc)
