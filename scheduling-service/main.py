...
app = FastAPI(title="Scheduling Service", version="0.1.0", docs_url=None, redoc_url=None)
...
app.include_router(event_schedule_router.router, prefix="/schedulings")


@app.get("/schedulings/swagger.yaml", include_in_schema=False)
async def swagger_spec():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/schedulings/swagger.yml", include_in_schema=False)
async def swagger_spec_alias():
    return FileResponse(swagger_path, media_type="application/yaml")


@app.get("/schedulings/docs", include_in_schema=False)
async def swagger_ui():
    return HTMLResponse(
        get_swagger_ui_html(
            openapi_url="/schedulings/swagger.yaml",
            title="Scheduling Service API Docs",
        ).body
    )
